		...
		c0,1.164,0.271,2.313,0.792,3.354l9.669,19.337H98.524z" />
                    ...
                    <td class="p-4">
                        <div class="flex flex-wrap justify-center gap-2">
                            @for($i=0; $i < count($clothe->images); $i++)
                                <button x-data="" x-on:click.prevent="$dispatch('open-modal', 'deleteImg-{{$clothe->images[$i]->idImg}}')" class="cursor-pointer relative group">
                                    <img src="{{$clothe->images[$i]->url}}" class="w-[60px] h-[80px] cursor-pointer object-cover" alt="">
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="white" class="w-8 h-8 absolute top-[25px] left-[15px] bg-red-500 rounded-full group-hover:opacity-90 opacity-0 duration-500 ease-in-out">
                                        <path stroke-linecap="round" stroke-linejoin="round" d="M14.74 9l-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 01-2.244 2.077H8.084a2.25 2.25 0 01-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 00-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 013.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 00-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 00-7.5 0" />
                                    </svg>
                                </button>
                                <x-modal name="deleteImg-{{$clothe->images[$i]->idImg}}" focusable>
                                    <form method="POST" action="{{ route('admin.deleteImg',$clothe->images[$i]->idImg) }}" class="p-6">
                                        @csrf
                                        @method('delete')
                                        ...
                                        <div class="w-full flex justify-center">
                                            <img src="{{$clothe->images[$i]->url}}" class="w-[200px] h-[300px]" alt="">
                                        </div>
                    ...
                    <td class="px-4 py-2 flex items-center h-[100px]">
                        <button x-data="" x-on:click.prevent="$dispatch('open-modal', 'createImg-{{$clothe->id}}')">
                        ...
                        <x-modal name="createImg-{{$clothe->id}}" focusable>
                            <form method="POST" action="{{ route('admin.uploadImg') }}" class="p-6">
                                @csrf
                                <h2 class="text-lg font-medium text-gray-900 dark:text-gray-100">
                                    Añadir imagen a {{$clothe->type_product}} {{$clothe->name}}
                                </h2>
                                <input type="text" id="idClo" name="idClo" wire:model="idClo" value="{{$clothe->id}}" class="form-input hidden" />
                                <div class="my-6">
		...
		c0,1.164,0.271,2.313,0.792,3.354l9.669,19.337H98.524z" />
                    ...
                    <td>
                        <div class="flex justify-center w-full flex-wrap gap-4">
                            @foreach($clothe->clothingSize as $sizes)
                    ...
                    <td>
                        <div class="flex justify-center w-full flex-wrap gap-6">
                            @foreach($clothe->clothingColor as $colors)
                            <span class="w-[25px] h-[25px] rounded-full border-2 border-black" style="background-color: #{{$colors->color}};"></span>
                            @endforeach
                        </div>
                    </td>
                    <td>
                        <button x-data="" x-on:click.prevent="$dispatch('open-modal', 'edit-clothe-{{$clothe->id}}')" class="cursor-pointer">
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6 hover:stroke-green-400">
                                <path stroke-linecap="round" stroke-linejoin="round" d="M16.862 4.487l1.687-1.688a1.875 1.875 0 112.652 2.652L10.582 16.07a4.5 4.5 0 01-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 011.13-1.897l8.932-8.931zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0115.75 21H5.25A2.25 2.25 0 013 18.75V8.25A2.25 2.25 0 015.25 6H10" />
                            </svg>
                        </button>
...
@endif
@else
<div class="text-2xl text-center font-semibold uppercase w-11/12 p-4 border-2 border-[#B4C3C9] rounded-md">No se han encontrado ningun resultado</div>
@endif